...
fys::cl::Map::Map(const std::string &tmxMap) {
    tmx::Map map;
    map.load(tmxMap);
...
void fys::cl::Map::initCollisionMapFromLayer(unsigned xMap, unsigned yMap, const tmx::TileLayer &tileLayer) {
    ...
    for (unsigned y = 0; y < yMap; ++y) {
        ...
        for (unsigned x = 0; x < xMap; ++x) {
            ...
            if (tiles.at(index).ID) {
                ...
                _mapElems[y][x].setTypeElem(fys::cl::MapElemProperty::BLOCK);
            }
        }
    }
}

fys::cl::MapElemProperty fys::cl::Map::getMapElementPropertyAtPosition(float x, float y) const {
    if (x >= _boudaryX || y >= _boudaryY || y < 0 || x < 0)
        return fys::cl::MapElemProperty::BLOCK;
    return _mapElems.at(y).at(x).getProperty();
}

void fys::cl::Map::triggerForPlayer(float x, float y, fys::cl::PlayerMapData &playerData) {
    if ((x >= 0 && x < _boudaryX) && (y >= 0 && y < _boudaryY)) {
        if (_mapElems.at(y).at(x).getProperty() == fys::cl::MapElemProperty::TRIGGER) {
